...
让我们考虑⼀个常⻅的简单情形，即编码器和解码器的隐藏单元个数均为 h，且函数 $a(s,h)=s^Th$ 。假设我们希望根据解码器单个隐藏状态 st′−1 和编码器所有隐藏状态 ht, t = 1, . . . , T来计算背景向量 ct′ 。我们可以将查询项矩阵 Q 设为![](./images/2019-8-31_18-29-41.png)，并令键项矩阵 K 和值项矩阵 V 相同且第 t ⾏均为 $h_t^T$ 。此时，我们只需要通过⽮量化计算：

$$
softmax(QK^T)V
$$

即可算出转置后的背景向量![](./images/2019-8-31_18-31-58.png)。当查询项矩阵 Q 的⾏数为 n 时，上式将得到 n ⾏的输出矩阵。输出矩阵与查询项矩阵在相同⾏上⼀⼀对应。

### 2.2 更新隐藏状态

现在我们描述第⼆个关键点，即更新隐藏状态。以⻔控循环单元为例，在解码器中我们可以对⻔控循环单元（GRU）中⻔控循环单元的设计稍作修改，从而变换上⼀时间步 t′−1 的输出 yt′−1、隐藏状态 st′−1 和当前时间步t′ 的含注意⼒机制的背景变量 ct′。解码器在时间步 t’ 的隐藏状态为：

![](./images/2019-8-31_18-32-47.png)

其中的重置⻔、更新⻔和候选隐藏状态分别为：

![](./images/2019-8-31_18-33-22.png)
...
![](./images/2019-9-25_20-18-36.png)

每个Ci可能对应着不同的源语句子单词的注意力分配概率分布，比如对于上面的英汉翻译来说，其对应的信息可能如下：

![](./images/2019-9-25_20-49-11.png)
...
1. **数据预处理**
   ...
   首先先下载本目录的数据和代码，并执行 [**datautil.py**](./datautil.py)，生成中、英文字典。

2. 执行 [**train.ipynb**](./train.ipynb)，训练时间会比较长。

3. 测试模型，运行[**test.py**](./test.py)文件。
